...
- **Round 1 — Build (analyst):** turn an idea into a spec. Terminal status is **always `draft`** — it must be reviewed before it can be implemented. This round never writes a plan.
...
Create a task for Step 0 and resolve it before anything else. Pick the mode from the entry signal:

- **Review mode** — the user pointed at an existing draft spec (a path, "review the spec", "developer round"), or there is exactly one draft spec under `docs/superpowers/specs/active/` for this topic.
- **Build mode** — the entry is an idea/context with no existing draft spec ("build a spec", "analyst round", a new feature to design).
...
Create a task for each item and complete them in order. Items 7–11 branch by mode.
...
3. **Load the existing spec (review mode only)** — read the draft spec end to end; note its `Status` and its Open Questions.
...
8. **Template fit check** — if a project-level `docs/superpowers/spec-template.md` exists, map the design onto it; surface every mismatch in one message and ask before deviating. Skip if absent.
9. **Write/update the spec** — save to `docs/superpowers/specs/active/YYYY-MM-DD-<topic>-design.md`, set the `Status` line per mode (build → `draft`; review → `draft` or `in_progress`), and maintain the Open Questions section. Commit.
...
**Spec status:** the spec's `Status` line is **always `draft`** at the end of build mode.

**Terminal (item 12 in build mode):** after the user reviews the spec, **stop**. Do NOT invoke `writing-plans`. Do NOT write code. The deliverable is a draft spec handed off to the developer review round. Tell the user: *"Draft spec ready at `<path>` (Status: draft). Hand it to a developer to run the review round."*

> Build mode's deliverable is a draft spec — write it after the design is approved. For a trivial change the user may decline the spec; then there is no handoff and no review round.
...
**Spec status (item 9):** decide at the end:
- **Leave `Status: draft`** if Open Questions remain unresolved/unaccepted, or the reviewer is not ready to approve.
- **Set `Status: in_progress`** (approve) only when **every Open Question is resolved or explicitly accepted** — i.e., every item is `- [x]` with a recorded resolution.

**Terminal (item 12 in review mode):**
- If leaving `draft`: after the user reviews the spec, **stop**. No `writing-plans`, no code.
- If approving (`in_progress`): invoke the `writing-plans` skill to create the implementation plan. Do NOT invoke any other skill.
...
Rules:
- **Never delete** a resolved item. Mark it `- [x]` and record the resolution so the decision history survives. Resolved decisions must also be reflected in the spec body.
...
- To promote a spec to `in_progress`, **every** item must be `- [x]`. Unchecked items block promotion; the spec stays `draft`.
...
- Review mode leaves `draft` or writes `in_progress` on approval (gated on Open Questions).
...
1. **Status correctness:** is the `Status` line present and right for the mode? Build → `draft`. Review → `draft` or `in_progress`, and `in_progress` only if every Open Question is `- [x]`.
...
- **Status is honest** — `draft` until genuinely approved; never promote with unresolved Open Questions.
