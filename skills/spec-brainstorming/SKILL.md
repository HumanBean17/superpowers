...
2. **Specs carry WHY, WHAT, and WHERE — not HOW.** The spec states the rationale (WHY); the behavior, contracts at schema level (JSON Schema), and decisions (WHAT); and anchors to the system's stable surface (WHERE): services, endpoints, event/topic names, tables, config keys, DTO fields. It MUST NOT carry HOW — invented class/method/file names for artifacts that don't yet exist (a new contract's *schema* is WHAT; its *carrier class* is HOW), signatures, per-class change lists, enumerated test cases, observability wiring, or implementation logic (method bodies, algorithms, actual code) — that is the plan's job. Existing code may be anchored at surface level, plus a single interior name where the decision is genuinely about that artifact — never inventories. Gather the spec's WHERE anchors in one touchpoint block (modular specs: `00-index.md` territory) so a plan agent starts from touchpoints, not from zero. Your job here is to design.
3. **Same depth for both roles — never role-gate detailization.** The line between a gap an analyst would catch and one a developer would catch is thin, so both rounds see the same data and may go as deep as the conversation needs. Never mute detail based on who is driving: "I'm an analyst, skip the Java class names" and "I'm a developer, skip the JSON schema" are both wrong. When something genuinely can't be answered in the current round, defer it to **Open Questions** rather than skipping or guessing. This governs how deep the *conversation* goes; what the written body binds to is Principle #2.
4. **Calibrate to a human-reviewable altitude.** The spec is the reviewer's surface; the plan is the agent-facing artifact that carries per-task detail (what one task needs to be written, not the design itself). The spec template sets the spec's sections; this governs only their altitude. Keep each section at design altitude — if it sprawls into per-task detail a human would skim rather than review, push it down into the plan, not into the spec — and never into code (Principle #2). This sets the *written spec's* altitude; Principle #3 still governs how deep the *conversation* goes — explore fully, then capture at review altitude, deferring per-task detail to the plan. But keep it concrete enough that a plan agent could expand the spec into tasks. Test design — what each test verifies and the expected result — is plan content (`writing-plans` owns it); the spec carries testing strategy only.
5. **The spec describes one change to the system, not a log of its own revisions.** The body states the target design of the change in the present tense and is **rewritten in place** as the design evolves — never as a changelog of the document (no "Deleted:", "Removed:", "Changed:", "Previously:", before/after, edit-dates). The document's revision history is git commits + the archived ADR. *Open Questions is the exception: it records decisions by design.* This governs build-mode iteration on an existing draft too — rewrite the body, don't annotate it. "Rewritten in place" means *edited* in place so the result reads as one flat present-tense design — never regenerated wholesale. Make the smallest edit that makes the body true; sections the change doesn't touch stay byte-identical — the diff is the reviewer's surface, and regenerating untouched text from memory is how contradictions enter.
6. **Sections are self-contained.** A decision is explained where it applies, not referred to by an opaque code ("per H1", "constraint C2"). Readers cannot hold an index in their heads, and every cross-reference code is a sync obligation that cascades on change. Some repetition between sections is acceptable — plans accept duplication for the same reason.
...
4. **Explore project context** — check files, docs, recent commits; read any `docs/superpowers/domain/` files matching the topic — they are the canonical domain reference to link, never re-embed.
...
8. **Template fit check** — if a project-level `docs/superpowers/spec-template.md` exists, map the design onto it; surface every mismatch in one message, resolved as a deviation or a template-flag proposal. Skip if absent. In review mode, re-check the draft against the template and surface any deviations the analyst introduced.
...
10. **Spec self-review** — quick inline check for placeholders, contradictions, ambiguity, scope, HOW leakage, status correctness, and Open Questions integrity (see below).
...
**Re-run discipline (iterating an existing draft):** list the sections the incoming change implies; edit only those (untouched sections stay byte-identical); then make one read-only consistency pass over the whole body, fixing contradictions the edit exposed. Commit with a delta brief in the message — requirement change → sections touched → Open Questions added/resolved.
...
**Walk the draft section by section** (item 7), from the highest-level section (architecture, goals) down to the most detailed (contracts, data shapes, error handling). **Size gate first:** if the draft is large enough that you cannot hold its cross-references while reading it, raise decomposition (splitting the spec) with the user *before* refining details — every downstream agent will strain the same way. For each section:
- Summarize what's there in a sentence or two, so the user knows what you read.
- Flag gaps, ambiguity, internal contradictions, missing error/edge cases, and any HOW that leaked in (spec must carry WHY/WHAT/WHERE — Core Principle #2).
...
3. **Template conformance** (if `docs/superpowers/spec-template.md` is present): required sections present, optional ones filled or dropped. A structural mismatch here raises the template flag instead of force-fitting the spec.
4. **HOW leakage:** did implementation logic or HOW-shaped content sneak in — method bodies, algorithms, signatures, file paths, per-class change lists, enumerated test cases, metrics/log config (Core Principle #2)? Push it into the plan; keep design, contracts, and anchors.
5. **Altitude check:** per-task detail a human would skim rather than review (what one task needs, not the design itself) belongs in the plan. A template section whose content tracks the code rather than the requirements — test scenarios, per-service file/class lists, observability mechanics — cannot be written at design altitude: that is a structural template flag (see the fit check), not a license to keep HOW in the spec. Then confirm the spec still names the components, contracts, and key decisions a plan agent needs to expand it into tasks — abstract, not vague.
...
12. **Coupling check:** for each major decision or constraint — if it changed, how many sections would need editing? More than a couple means it is referenced instead of restated (Core Principle #6): inline the rationale where it applies. Reference material that predates or outlives this change (domain code tables, status semantics, filtering rules) embedded in the body → extract to a domain reference file under `docs/superpowers/domain/` and link it.
...
Specs are freeform unless the project provides `docs/superpowers/spec-template.md` — a project-level file read at spec-writing time and written only through a template-improvement proposal the user approves in the fit check. It shapes only the written spec, never the conversation.
...
**Fit check.** First classify every template section — WHAT (design) / HOW (content that tracks the code, not the requirements) / domain reference. A HOW section (test plans, per-service change lists, observability mechanics) raises the **structural** flag immediately, cleanly fillable or not — a fillable HOW section never mismatches, so waiting for a mismatch means waiting forever; a domain-reference section proposes the domain-file home. Then map the design onto the template. Drop non-required sections silently. Surface every mismatch — a `[required]` section that doesn't fit, a section the task needs but the template lacks, any conflict — in one batched message, each classified:

- **Incidental** — a quirk of this one design. Propose a deviation for this spec.
- **Structural** — it would recur for this class of tasks, or the template demands what a core principle forbids (implementation code, per-task detail below spec altitude, or a section whose content tracks the code rather than the requirements). Raise the **template flag**: propose the concrete template edit — add, rename, drop, or re-`[required]` a section.

Ask before applying any resolution; the user can reclassify any item. An approved template edit lands in the same session: update `spec-template.md`, commit it, and write the spec conformant to the updated template. A declined edit falls back to a deviation for this spec. In review mode, an analyst deviation that is structural is a template smell — re-check it through the same flag.
