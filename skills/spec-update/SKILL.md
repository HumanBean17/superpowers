...
2. **Specs carry WHY, WHAT, and WHERE — not HOW.** Rationale (WHY); behavior, contracts at schema level, and decisions (WHAT); anchors to the system's stable surface (WHERE) — services, endpoints, event/topic names, tables, config keys, DTO fields — gathered in one touchpoint block. No HOW: invented class/method/file names for artifacts that don't yet exist (a contract's schema is WHAT; its carrier class is HOW), signatures, per-class change lists, enumerated test cases, observability wiring, method bodies, algorithms, or actual code — that is the plan's. Existing code anchors at surface level, plus a single interior name where the decision is genuinely about that artifact — never inventories.
3. **The spec describes one change to the system, not a log of its own revisions.** The body states the target design of the change (what gets added, changed, or removed, and why) in the present tense. As the design evolves it is **rewritten in place** to keep that single description current and complete. The body must NOT become a changelog of the document — no "Deleted:", "Removed:", "Changed:", "Previously:", no before/after, no edit-dates. The spec describes that one change; the document's revision history lives in git commits and the archived ADR. *Open Questions is the only exception: it records decisions by design, not document revisions.* "Rewritten in place" means *edited* in place so the result reads as one flat present-tense design — never regenerated wholesale; sections the change doesn't touch stay byte-identical, and the diff is the reviewer's surface.
...
5. **Sections are self-contained.** A decision is explained where it applies, not referred to by an opaque code ("per H1", "constraint C2") — every cross-reference code is a sync obligation that cascades on change. Some repetition between sections is acceptable; plans accept duplication for the same reason.
...
After rewriting the body, look at it with fresh eyes. Run the standard checks — status correctness, Open Questions integrity, template conformance, HOW leakage, placeholder scan, internal consistency, scope, ambiguity, coupling — **and** the snapshot checks:
...
6. **Coupling:** for each major decision or constraint — if it changed, how many sections would need editing? More than a couple means it is referenced instead of restated (Core Principle #5): inline the rationale where it applies. Reference material that predates or outlives this change (domain code tables, status semantics, filtering rules) embedded in the body → extract to a domain reference file under `docs/superpowers/domain/` and link it.
