...
The `brainstorming` and `spec-brainstorming` skills now state an explicit abstraction-level calibration: the **spec is the human's review surface**, kept at design altitude (components, contracts, decisions); the **plan is the agent-facing artifact** that carries per-task detail (exact signatures, data shapes, validation, test scenarios). When a spec section piles up detail a human would skim rather than review, the guidance is to push it down into the plan — not grow the spec, never write code in it. A new self-review "Altitude check" enforces this; a lower bound keeps the spec concrete enough (named components, contracts, key decisions) that a plan agent can still expand it into tasks.
