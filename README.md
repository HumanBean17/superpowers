...
A new opt-in **spec-update** skill (sibling to spec-brainstorming) reconciles or evolves an existing spec — `draft`, `in_progress`, or `implemented`. **Reconcile** rewrites the body to match built reality (scope unchanged, no plan); **evolve** folds in new scope, then plans the delta in plan mode and implements it on approval (`implemented → in_progress`) — bypassing `writing-plans`; the workflow is not waterfall. On a **draft** both intents are draft-mapped and the session is **body-only** — no plan, no code, an unapproved design never builds: reconcile refines and re-aligns the draft with the codebase it describes, evolve folds in new scope, and at the terminal the skill offers `draft → in_progress` (accepted only when every Open Question is resolved or accepted, and never followed by a `writing-plans` invocation). spec-update is the no-ceremony editor for targeted draft edits; full build/review rounds stay with spec-brainstorming, which carries the same snapshot principle and self-review check. Both run under a **snapshot gate**: the body describes one change to the system, rewritten in place, never a changelog of itself (no `Deleted:`/`Changed:`/before-after); the document's history is git + the archived ADR. The regular `brainstorming` skill now writes a `Status` line and promotes `draft → in_progress` before handing off to `writing-plans`.

**Rationale:** Real work needs changes after a spec and plan are implemented, but ad-hoc prompt edits corrupted specs — agents treated them as logs and drifted the detailization level. Explicit terminal/implemented statuses plus a protected update skill with the snapshot gate keep specs honest across the whole non-waterfall lifecycle. Editing a draft previously required a full spec-brainstorming round; the no-ceremony draft path covers targeted edits while promotion stays gated on resolved Open Questions.
...
### Fit Check Can Flag the Template Itself

The template fit check in `brainstorming` and `spec-brainstorming` now classifies every mismatch as **incidental** (a quirk of this one design → deviate on this spec, as before) or **structural** (it would recur for this class of tasks, or the template demands what a core principle forbids — implementation code, per-task detail below spec altitude). A structural mismatch raises the **template flag**: the agent tells the user the template doesn't fit or needs improvement, proposes the concrete template edit (add, rename, drop, or re-`[required]` a section), and on approval edits `spec-template.md` in the same session, commits it, and writes the spec conformant to the updated template; a declined edit falls back to a deviation for the current spec, and the user can reclassify any item. The template's ownership rule changes accordingly — "read at spec-writing time, never written to" becomes "written only through a template-improvement proposal the user approves in the fit check". The flag fires at the fit check in both spec-brainstorming rounds (an analyst deviation that is structural is a template smell in the review round), with a backstop in each skill's spec self-review; `spec-update` inherits the protocol through its fit-check reference.

**Rationale:** Templates are authored once and fit the first few tasks; as task types evolve, agents silently absorbed the drift — each spec deviating a little more, quality eroding, the template never improving. The flag closes that loop with the human approving every template change.
...
**Rationale:** Teams using rich custom templates produced specs so large that any agent fetching or editing them blew its context. Splitting the spec at the project's own section seams — with an always-small index as the entry point — lets authoring and planning touch only the part they need, while keeping `Status` and `Open Questions` centralized in the index so the lifecycle machinery stays in one place.
...
### Specs Carry WHY, WHAT, WHERE — HOW Is the Plan's

In `spec-brainstorming` and `spec-update`, Core Principle #2 is rewritten from "specs carry design, not code" (which allowed referencing classes, methods, and file paths) to an explicit boundary: the spec carries **WHY** (rationale), **WHAT** (behavior, contracts at schema level, decisions), and **WHERE** (anchors to the system's stable surface — services, endpoints, event/topic names, tables, config keys, DTO fields — gathered in one touchpoint block so a plan agent starts from touchpoints, not from zero). It never carries **HOW**: invented class/method/file names for artifacts that don't yet exist (a new contract's schema is WHAT; its carrier class is HOW), signatures, per-class change lists, enumerated test cases, or observability wiring — that is the plan's. Existing code is anchored at surface level, plus a single interior name where the decision is genuinely about that artifact — never inventories. Principle #3 gains a reconciliation clause (conversation depth ≠ written binding), Principle #4 states test-design ownership (`writing-plans` owns it; the spec carries testing strategy only), and a new principle — **sections are self-contained** — bans opaque cross-reference codes ("per H1", "constraint C2") in favor of inline rationale. The self-review "Code leakage" check becomes **"HOW leakage"**, and the altitude check's template-supremacy carve-out ("anything the template reserves a section for stays") is gone: a template section whose content tracks the code rather than the requirements is a structural template flag, not a license to keep HOW. The fit check now classifies every template section (WHAT / HOW / domain reference) up front and flags HOW sections immediately — cleanly fillable or not.

**Rationale:** An enterprise postmortem (#10) traced a spec's degradation into an ~1100-line monolith to mixed change-frequency concerns: the old "design, not code" rule licensed class/path inventories, and the template carve-out let five HOW-only sections survive every gate.

### Spec Hygiene: Coupling Check, Surgical Draft Edits

The two spec skills gain one combined **coupling check** in self-review: for each major decision — if it changed, how many sections would need editing? More than a couple means it is referenced instead of restated: inline the rationale where it applies; embedded domain reference material → extract to `docs/superpowers/domain/` and link. The snapshot principle is clarified as a property, not a procedure: "rewritten in place" means *edited* in place (untouched sections stay byte-identical — the diff is the reviewer's surface), never regenerated wholesale. `spec-brainstorming` build-mode re-runs follow an explicit discipline — list the sections the incoming change implies, edit only those, one read-only consistency pass, commit with a delta brief (requirement change → sections touched → Open Questions resolved) — and `spec-update` extends its modular surgical rule to single-file specs: edit only the sections this update touches. The review round opens with a size gate: if the reviewer cannot hold the draft's cross-references while reading, decomposition is raised before details are refined.

**Rationale:** Full-body regeneration was the hallucination vector (rewrite from partial memory → contradictions → bigger rewrites) and uninformative diffs left reviewers blind; smallest-true-edit plus narrated deltas fixes both using git, the history surface the kit already blesses.

### Domain Reference Files Survive Archiving

A new artifact class: `docs/superpowers/domain/<topic>.md` — present-state domain reference (operation-code tables, status semantics, filtering rules). Specs link, never embed; domain files change on their own cadence, independent of any spec. `finishing-a-development-branch` never archives them with a released spec: they are current domain state, not decision records, so the next spec touching the same domain reuses them (the coupling check extracts to them).

**Rationale:** Domain tables embedded in a feature spec died with it at archive time (spec → ADR), forcing copy-paste into every next spec — the pattern needed a home that outlives any one spec.
...
- **spec-update** - Reconcile/evolve an existing draft/in_progress/implemented spec (snapshot-gated; no-ceremony draft edits with terminal promotion; plan-mode + direct implement for new scope)
